 ...
 */
void aggregate_results_atomic(int* global_TF, int* local_TF) {
...
int main(int argc, char* argv[]) {
    ...
    for (int gene_index = 0; gene_index < genes.num_genes; ++gene_index) {
        ...
        aggregate_results_atomic(TF, gene_TF);
        free(gene_TF);
    }
    ...
    for (int i = 0; i < NUM_TETRANUCS; ++i) {
        fprintf(s.output, "%.6f", average_TF[i]);
        if (i < NUM_TETRANUCS - 1) fprintf(s.output, "\n");
    }
