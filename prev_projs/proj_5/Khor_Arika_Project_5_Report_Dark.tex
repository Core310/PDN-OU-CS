...
\subsection*{Algorithm Description}
A classic embarrassingly parallel problem where each process generates independent random samples within a unit square to estimate the area of a circle. 
...
\section*{\color{white}Deep Performance Analysis and MPI Bottlenecks}
...
\begin{itemize}
    ...
    \item \textbf{Initialization Noise:} In Monte Carlo ($N=2^{16}$), the ~110ms execution time is dominated by MPI process spawning rather than the negligible computation.
\end{itemize}
