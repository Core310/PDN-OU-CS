...
int main(int argc, char* argv[]) {
    ...
    cuda_ret = cudaDeviceSynchronize();
    err_check(cuda_ret, (char*)"error in hash kernel execution", 7);
    ...
    int reduction_block = ceil((float)trials / (float)((BLOCK_SIZE) * 2));
    dim3 reduceGrid(reduction_block, 1, 1);

    unsigned int* out_hash;
    unsigned int* out_nonce;

    cuda_ret = cudaMalloc((void**)&out_hash, reduction_block * sizeof(unsigned int));
    err_check(cuda_ret, "error in alloc out_hash", 8);
    cuda_ret = cudaMalloc((void**)&out_nonce, reduction_block * sizeof(unsigned int));
    err_check(cuda_ret, "error in alloc out_nonce", 9);    

    size_t shared_mem_size = 2 * BLOCK_SIZE * sizeof(unsigned int);

    reduction_kernel <<< reduceGrid, dimBlock, shared_mem_size >>>(
        device_hash_array, 
        device_nonce_array, 
        out_hash, 
        out_nonce,
    ...
    cuda_ret = cudaDeviceSynchronize();
    err_check(cuda_ret, "error in reduction kernel execution", 10);

    unsigned int* host_out_hash = (unsigned int*)calloc(reduction_block, sizeof(unsigned int));
    unsigned int* host_out_nonce = (unsigned int*)calloc(reduction_block, sizeof(unsigned int));
    
    cuda_ret = cudaMemcpy(host_out_hash, out_hash, reduction_block * sizeof(unsigned int), cudaMemcpyDeviceToHost);
    err_check(cuda_ret, "error in local hash min", 11);
    cuda_ret = cudaMemcpy(host_out_nonce, out_nonce, reduction_block * sizeof(unsigned int), cudaMemcpyDeviceToHost);
    err_check(cuda_ret, "error in local nonce min", 12);
    ...
    //update global mins after reduction
    for (int i = 0; i < reduction_block; i++){
        if (host_out_hash[i] < min_hash){
            min_hash = host_out_hash[i];
            min_nonce = host_out_nonce[i];
        }
    }
